...
//        }
        ...
        AdvancedBinarySearch<String> abs = new AdvancedBinarySearch<String>();
        String[] a = {"a", "b", "c", "d", "e", "f"};
        System.out.println(abs.advancedBinarySearch(a, "e", 0, a.length - 1));
...
class AdvancedBinarySearch<T extends Comparable<T>> {
    public int advancedBinarySearch(T[] a, T key, int low, int hi) {
        if (low > hi) throw new IllegalArgumentException("Low > hi!");
        if (low < 0 || hi >= a.length) throw new IllegalArgumentException("Incorrect indexes");
        int mid = 0;
        int iters = 0;
